...
export interface IStorage {
  ...
  updateImaging(
    id: string,
    updates: Partial<Imaging>
  ): Promise<Imaging | undefined>;
...
export class MemStorage implements IStorage {
  ...
  private initializeMockData() {
    ...
    const patient1: Patient = {
      ...
      medicalHistory:
        "No significant past medical history. Regular exercise, non-smoker.",
    ...
    const patient2: Patient = {
      ...
      medicalHistory:
        "Asthma since childhood, well-controlled with medication.",
    ...
    const case1: Case = {
      ...
      title: "Suspected minimal trauma fracture in wrist",
      description:
        "34-year-old female presents with wrist pain after fall. Initial imaging suggests possible minimal trauma fracture.",
      status: "analyzing",
      priority: "high",
      riskScore: 75,
      aiAnalysis: null,
    ...
    const case2: Case = {
      ...
      description:
        "Routine follow-up for asthma management and medication adjustment.",
      status: "pending",
      priority: "medium",
      riskScore: 45,
      aiAnalysis: null,
    ...
    const vitals1: Vitals = {
      ...
      painScale: 6,
      symptoms: [
        { name: "Wrist pain", severity: "moderate", duration: "2 days" },
      ],
    ...
    const imaging1: Imaging = {
      ...
      uploadedAt: new Date(),
      aiAnnotations: {
        findings: ["Possible minimal trauma fracture at distal radius"],
        confidence: 0.82,
      },
      radiologistNotes: "Subtle cortical disruption noted at distal radius",
      findings: "Possible minimal trauma fracture, recommend follow-up imaging",
      impression: "Suspicious for minimal trauma fracture of distal radius",
    ...
    const note1: Notes = {
      ...
      content:
        "S: 34F presents with right wrist pain after fall 2 days ago.\nO: Tenderness over distal radius, limited ROM\nA: Suspected minimal trauma fracture of right distal radius\nP: X-ray ordered, pain management, follow-up in 1 week",
    ...
    const order1: Orders = {
      ...
      results: { findings: "Possible minimal trauma fracture identified" },
  ...
  async getCaseWithDetails(id: string): Promise<CaseWithDetails | undefined> {
    ...
    const vitals = Array.from(this.vitals.values()).filter(
      (v) => v.caseId === id
    );
    const imaging = Array.from(this.imaging.values()).filter(
      (i) => i.caseId === id
    );
    ...
    const riskAssessments = Array.from(this.riskAssessments.values()).filter(
      (r) => r.caseId === id
    );
    ...
    return allCases.filter((caseItem) => {
      ...
      if (filters.riskScore && caseItem.riskScore !== null) {
        if (
          ...
          caseItem.riskScore < filters.riskScore.min
        ) {
          return false;
        }
        if (
          filters.riskScore.max &&
          caseItem.riskScore > filters.riskScore.max
        ) {
          return false;
        }
      }
  ...
  async getImagingByCase(caseId: string): Promise<Imaging[]> {
    return Array.from(this.imaging.values()).filter(
      (imaging) => imaging.caseId === caseId
    );
  }
  ...
  async getRiskAssessmentsByCase(caseId: string): Promise<RiskAssessment[]> {
    return Array.from(this.riskAssessments.values()).filter(
      (assessment) => assessment.caseId === caseId
    );
  }

  async createRiskAssessment(
    insertAssessment: InsertRiskAssessment
  ): Promise<RiskAssessment> {
    const id = randomUUID();
  ...
  async createAgent(insertAgent: InsertAiAgent): Promise<AiAgent> {
    ...
    const agent: AiAgent = {
      ...
      isActive:
        insertAgent.isActive !== undefined ? insertAgent.isActive : true,
    };
