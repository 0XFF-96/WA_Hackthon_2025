...
export function WorkflowGraph({ 
  ...
  const [edges] = useState<WorkflowEdge[]>([
    { id: 'e1', from: 'input', to: 'orchestrator', label: 'Patient data', status: 'completed' },
    { id: 'e2', from: 'orchestrator', to: 'diagnostician', label: 'Coordinate', status: isSimulating ? 'active' : 'pending' },
    { id: 'e3', from: 'orchestrator', to: 'radiologist', label: 'Coordinate', status: isSimulating ? 'active' : 'pending' },
    { id: 'e4', from: 'orchestrator', to: 'treatment', label: 'Coordinate', status: 'pending' },
    { id: 'e5', from: 'diagnostician', to: 'synthesis', label: 'Analysis', status: 'pending' },
    { id: 'e6', from: 'radiologist', to: 'synthesis', label: 'Imaging', status: 'pending' },
    { id: 'e7', from: 'treatment', to: 'synthesis', label: 'Plan', status: 'pending' },
    { id: 'e8', from: 'synthesis', to: 'output', label: 'Final plan', status: 'pending' }
  ]);
  ...
  return (
    <Card className="w-full h-[700px] flex flex-col" data-testid="card-workflow-graph">
        ...
        <div className="relative z-10 w-full h-full" style={{ minHeight: '600px' }}>
          {nodes.map((node) => {
            ...
            return (
              ...
              >
                <Card className={`w-60 hover-elevate transition-all duration-200 ${
                  ...
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          <h4 className="font-semibold text-xs" data-testid={`node-name-${node.id}`}>
                            {node.name}
                          </h4>
                        </div>
                      </div>
                      <div className="flex items-center space-x-1">
                        {StatusIcon && (
                          <StatusIcon className={`w-3 h-3 ${
                            node.status === 'active' ? 'text-primary animate-pulse' :
                            node.status === 'completed' ? 'text-green-600' :
                            'text-muted-foreground'
                          }`} />
                        )}
                        <Badge className={statusColors[node.status]} data-testid={`node-status-${node.id}`}>
                          {node.status}
                        </Badge>
                      </div>
                    </div>
