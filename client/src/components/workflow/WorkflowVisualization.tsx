...
import { WorkflowExecution, WorkflowStep, Agent, AgentInteraction } from '@/types/workflow';
...
export function WorkflowVisualization({ caseId, onWorkflowComplete }: WorkflowVisualizationProps) {
  ...
  const simulateWorkflow = (execution: WorkflowExecution) => {
    const workflowSteps = [
      {
        ...
        input: { caseId: execution.caseId, symptoms: 'Pain in left hip, difficulty walking' },
        duration: 2000
      },
      {
        ...
        input: { symptoms: 'Pain in left hip, difficulty walking', age: 65, history: 'Previous fall' },
        duration: 3000
      },
      {
        agentId: 'radiologist',
        agentName: 'RadiologyAI',
        action: 'Analyze imaging data for micro-fractures',
      ...
      },
      {
        ...
        input: { diagnosis: 'Suspected micro-fracture', riskFactors: ['age', 'previous fall'] },
        duration: 2000
      },
      {
        agentId: 'orchestrator',
    ...
    const processStep = () => {
      ...
      setTimeout(() => {
        ...
        setCurrentExecution(prev => prev ? {
          ...prev,
          steps: prev.steps.map(step => step.id === newStep.id ? completedStep : step)
        } : null);

        // Update agent status
        setAgents(prev => prev.map(agent => 
          agent.id === stepConfig.agentId 
            ? { ...agent, status: 'idle', currentTask: undefined, lastActivity: new Date() }
            : agent
        ));
        ...
        const interaction: AgentInteraction = {
          ...
          toAgent: stepIndex < workflowSteps.length - 1 ? workflowSteps[stepIndex + 1].agentId : 'system',
  ...
  const generateStepOutput = (agentId: string) => {
    const outputs = {
      ...
      diagnostician: { diagnosis: 'Suspected micro-fracture', confidence: 87, differential: ['stress fracture', 'bone bruise'] },
      radiologist: { findings: 'Subtle cortical irregularity', recommendation: 'Follow-up MRI' },
      treatment_planner: { plan: 'Conservative management', medications: ['pain relief'], followUp: '2 weeks' }
    };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Real-time AI agent collaboration and decision tracking</p>
        </div>
        <div className="flex gap-2">
          {!isRunning ? (
            <Button onClick={startWorkflow} className="bg-primary hover:bg-primary/90">
          ...
          <CardContent>
            <div className="space-y-2">
              {interactions.slice(0, 5).map((interaction) => (
                <div key={interaction.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                  <MessageSquare className="w-4 h-4 text-gray-600" />
                  <div className="flex-1">
                    <div className="text-sm">
                      <span className="font-medium">{interaction.fromAgent}</span>
                      <span className="text-gray-500"> → </span>
                      <span className="font-medium">{interaction.toAgent}</span>
                    </div>
                    <div className="text-xs text-gray-600">{interaction.message}</div>
                  </div>
