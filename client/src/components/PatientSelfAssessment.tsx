...
import { AssessmentData, AssessmentResult, AssessmentStep, AssessmentState } from '@/types/assessment';
...
export function PatientSelfAssessment({ onClose }: PatientSelfAssessmentProps) {
  ...
  const steps: { key: AssessmentStep; title: string; description: string }[] = [
    { key: 'landing', title: 'Welcome', description: 'Get started with your assessment' },
    { key: 'symptoms', title: 'Symptoms', description: 'Tell us about your pain' },
    { key: 'functional', title: 'Function', description: 'How does it affect your movement' },
    { key: 'risk-factors', title: 'Risk Factors', description: 'Your health background' },
    { key: 'recent-event', title: 'Recent Events', description: 'Any recent incidents' },
  ...
  const calculateRiskAssessment = (data: AssessmentData): AssessmentResult => {
    ...
    if (data.painWhenWalking) { score += 5; factors.push('Pain when walking'); }
    if (data.painWhenClimbing) { score += 5; factors.push('Pain when climbing stairs'); }
    if (data.limpingOrImbalance) { score += 10; factors.push('Limping or imbalance'); }

    // Risk factors (0-30 points)
    if (data.ageGroup === 'over-70') { score += 15; factors.push('Age over 70'); }
    else if (data.ageGroup === '50-70') { score += 10; factors.push('Age 50-70'); }
    
    if (data.osteoporosisHistory) { score += 10; factors.push('Osteoporosis history'); }
    if (data.previousFractures) { score += 8; factors.push('Previous fractures'); }
    if (data.smoking) { score += 5; factors.push('Smoking'); }
    if (data.lowCalciumIntake) { score += 5; factors.push('Low calcium intake'); }

    // Recent events (0-20 points)
    if (data.recentFall) { score += 15; factors.push('Recent fall'); }
    if (data.recentJump) { score += 10; factors.push('Recent jump'); }
    if (data.recentImpact) { score += 10; factors.push('Recent impact'); }
    ...
    if (score <= 30) {
      riskLevel = 'low';
      summary = 'Based on your responses, you have a low risk of micro-fracture.';
      recommendations = [
      ...
      riskLevel = 'medium';
      summary = 'Your responses suggest a moderate risk of micro-fracture.';
      recommendations = [
      ...
      riskLevel = 'high';
      summary = 'Your responses indicate a high risk of micro-fracture.';
      recommendations = [
